...
#include "../lock_functions/hle_asm_exch_lock-intel_store.h"
...
void printHeader(FILE * out = stdout) {
	fprintf(out,
			"Repeats;hle_asm_exch_lock-intel;hle_exch_lock-intel_store;hle_exch_lock-leis\n");
}
...
int main(int argc, char *argv[]) {
	...
	int lock_functions_length = 3;
	void (*lock_functions[])(
			type *lock) = {hle_asm_exch_lock, hle_asm_exch_lock_intel_store, hle_asm_exch_lock2};
	void (*unlock_functions[])(
			type *lock) = {hle_asm_exch_unlock, hle_asm_exch_unlock_intel_store, hle_asm_exch_unlock2};
		...
		for (int f = 0; f < lock_functions_length; f++) {
			...
			for (int l = 0; l < loops; l++) {
				...
				for (int t = 0; t < num_threads; t++) {
					...
					attrs->unlock_function = unlock_functions[f];
